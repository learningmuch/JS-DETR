...
class Backbone(BackboneBase):
    ...
    def __init__(self, name: str,
        ...
        backbone = resnet50(
            pretrain_path='./models/resnet50.pth')  # 创建一个resnet50的实例
...
class Joiner(nn.Sequential):
    ...
    def forward(self, tensor_list: NestedTensor):
        ...
        for name, x in xs.items():
            out.append(x)
            # position encoding
            # # 使用第二个子模块（position_embedding）生成位置编码
            # self[1] 其实就是PositionEmbeddingSine()
            # 并将位置编码转换为与输出相同的数据类型（x.tensors.dtype）
...
#         xs = self[0](tensor_list)
#         out: List[NestedTensor] = []
#         pos = [None] * len(xs)  # 不再获取位置编码，将pos长度设置为xs的长度，并填充为None
#         for name, x in xs.items():
#             out.append(x)
#
