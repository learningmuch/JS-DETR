...
def get_args_parser():
    ...
    # 检测的图像路径
    # 检测结果保存路径
    # parser.add_argument('--output_dir', default=r'E:demo\output\res50',
    #                     help='path where to save, empty for no saving')
    # 存放模型的位置
    # parser.add_argument('--resume', default=r'weight_all/DA_ciou/test/checkpoint_best_epoch61.pth',
    ...
    parser.add_argument('--output_dir', default=r'E:demo\output1',
                        help='path where to save, empty for no saving')
...
def main(args):
    ...
    for image_item in image_file_path:
        ...
        gt_boxs = []
        tree = ET.parse(os.path.join(args.source_dir_1, image_item[:-4] + ".xml"))
        root = tree.getroot()
        ...
        # box = boxes.
        # box_list = [[box[0][0], box[0][1], box[0][2], box[0][3]] for box in boxes]
        boxes_list = [box.tolist() for box in boxes]
        for bbox in boxes_list:
            for gt_box in gt_boxs:
                if calIoU(bbox, gt_box) >= 0.5:
                    tp += 1
                    with open(os.path.join(args.save_dir, "tp.txt"), "a") as f:
                        f.write(image_item +'\n')
                    break
                # elif calIoU(bbox, gt_box) < 0.5:
                #     fp+=1
                if gt_box == gt_boxs[-1]:
                    fp += 1
                    with open(os.path.join(args.save_dir, "fp.txt"), "a") as f:
                        f.write(image_item +'\n')
        ...
        # cv2.waitKey()
        cv2.imwrite(os.path.join(args.output_dir, image_item), image)
